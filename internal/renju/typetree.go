// Package renju implements the Renju-Basic Gomoku rule variant.
package renju
...
const (
	statusLength  = 10
	analyzeLength = statusLength + 1 // 11
)
...
// InitTypeTree initializes the renju type tree. Safe for concurrent use.
func InitTypeTree() {
...
// ClassifyType classifies a status array into a ChessType using the renju type tree.
...
func plantTree() {
	...
	dfs(typeTreeRoot, status, analyzeLength/2, -1, 0, 0, false, false)
}

func dfs(node *gomoku.TypeNode, status []board.StoneStatus, location, move int,
	blackConnect, whiteConnect int, blackBlock, whiteBlock bool) {
	...
	if (blackBlock && whiteBlock) || status[location] == board.Bound ||
		...
		// Track consecutive same-color stones to prevent existing fives
		switch status[location] {
		case board.Black:
			blackConnect++
		case board.White:
	...
	for i := 0; i < 4; i++ {
		// Skip placing same color if connect >= 4 (prevents existing five in tree)
		if (i == 0 && blackConnect >= 4) || (i == 1 && whiteConnect >= 4) {
			continue
		}
		node.Children[i] = &gomoku.TypeNode{}
		status[location] = stones[i]
		dfs(node.Children[i], status, location, move,
...
func typeAnalyze(status []board.StoneStatus, color board.StoneStatus, checkLevel bool) gomoku.SingleType {
	...
	if connect > 5 {
		// Overline: forbidden for black, counts as five for white
		if color == board.Black {
			return gomoku.SingleType{Length: -1, Life: 0, Level: 0}
		}
	...
	if lType.Length == 5 && rType.Length == 5 {
		if connect == 4 {
			// Both sides produce five with connect==4 → live four
			return gomoku.SingleType{Length: 4, Life: 1, Level: 0}
		}
		// Otherwise → forbidden (double four creating overline)
		return gomoku.SingleType{Length: -1, Life: 0, Level: 0}
